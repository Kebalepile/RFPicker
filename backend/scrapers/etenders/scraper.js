...
/** ---------- Helpers to parse expanded detail row ---------- **/

// Normalize a label to a JS-friendly key
function normalizeKey(k) {
  return k
    .toLowerCase()
    .replace(/\(.*?\)/g, '')       // drop bracketed hints
    .replace(/[^a-z0-9]+/g, ' ')   // spaces
    .trim()
    .replace(/\s+/g, '_');         // snake_case
}

// Convert lines into both normalized and original-label maps.
// Supports separators: colon (:), tabs, or 2+ spaces (and optional hyphen).
function kvObjectDual(lines) {
  const norm = {};
  const raw = {};

  let currentKeyNorm = null;
  let currentKeyRaw = null;

  for (const rawLine0 of lines) {
    // normalize whitespace (convert non-breaking spaces too)
    const rawLine = rawLine0.replace(/\u00A0/g, ' ').trim();
    if (!rawLine) continue;

    // Accept "Key : Value" OR "Key<TAB>Value" OR "Key  Value" OR "Key - Value"
    const m = rawLine.match(/^(.+?)(?:\s*[:\-]\s*|\t+|\s{2,})(.*)$/);
    if (m) {
      const labelRaw = m[1].trim();
      const value = m[2].trim();

      const keyNorm = normalizeKey(labelRaw);
      norm[keyNorm] = value;
      raw[labelRaw] = value;

      currentKeyNorm = keyNorm;
      currentKeyRaw = labelRaw;
    } else if (currentKeyNorm) {
      // continuation line for the previous key
      norm[currentKeyNorm] = `${norm[currentKeyNorm]}\n${rawLine}`.trim();
      raw[currentKeyRaw] = `${raw[currentKeyRaw]}\n${rawLine}`.trim();
    }
  }
...
// Return slice of lines between a start header and next header (exclusive)
function sectionSlice(lines, headerRegex, nextHeaderRegex) {
  const startIdx = lines.findIndex(l => headerRegex.test(l));
  if (startIdx === -1) return [];
  let endIdx = lines.length;
  if (nextHeaderRegex) {
    for (let i = startIdx + 1; i < lines.length; i++) {
      if (nextHeaderRegex.test(lines[i])) { endIdx = i; break; }
    }
  }
...
// Extract the four sections as text arrays, then convert to objects
function parseSections(detailText) {
  const lines = (detailText || '')
    .split('\n')
    .map(s => s.replace(/\r/g, '').trimEnd());

  const R_DETAILS = /^details\s*:?\s*$/i;
  const R_ENQ     = /^enquiries\s*:?\s*$/i;
  const R_BRIEF   = /^briefing\s*session\s*:?\s*$/i;
  const R_DOCS    = /^tender\s*documents\s*:?\s*$/i;

  const detailsLines   = sectionSlice(lines, R_DETAILS, R_ENQ);
  const enquiriesLines = sectionSlice(lines, R_ENQ, R_BRIEF);
  const briefLines     = sectionSlice(lines, R_BRIEF, R_DOCS);
  const docsLines      = sectionSlice(lines, R_DOCS, null);

  const d  = kvObjectDual(detailsLines);
  const eq = kvObjectDual(enquiriesLines);
  const br = kvObjectDual(briefLines);

  return {
    details: d.norm,
    details_labels: d.raw,              // ← original labels preserved

    enquiries: eq.norm,
    enquiries_labels: eq.raw,           // ← original labels preserved

    briefingSession: br.norm,
    briefingSession_labels: br.raw,     // ← original labels preserved

    _docsText: docsLines.join('\n').trim()
  };
}
...
(async () => {
  ...
  while (hasNextPage) {
    ...
    for (let pIdx = startIdx; pIdx < parentIdx.length; pIdx++) {
      ...
      // Parse sections
      const sections = parseSections(detailData.text || '');

      // Also keep some top-level convenience fields for compatibility
      ...
      const record = {
        source: 'dom-details',
        tenderNumber: (sections.details?.tender_number) || (tenderNoMatch ? tenderNoMatch[1].trim() : null),
        title,
        category,
        advertisedDate: (sections.details?.date_published) || (datePublishedMatch ? datePublishedMatch[1].trim() : (advertised || null)),
        closingDate: (sections.details?.closing_date) || (closingDateMatch2 ? closingDateMatch2[1].trim() : (closingRaw || null)),
        buyerName: (sections.details?.organ_of_state) || (buyerMatch ? buyerMatch[1].trim() : null),
        eSubmission: /yes|accept/i.test(eSub) ? 'Yes' : (eSub || null),
        tenderType: (sections.details?.tender_type) || (tenderTypeMatch ? tenderTypeMatch[1].trim() : null),
        province: (sections.details?.province) || (provinceMatch ? provinceMatch[1].trim() : null),
        datePublished: (sections.details?.date_published) || (datePublishedMatch ? datePublishedMatch[1].trim() : null),

        // NEW: full structured sections you asked for
        details: sections.details || {},
        enquiries: sections.enquiries || {},
        briefingSession: sections.briefingSession || {},
        tenderDocuments: (detailData.anchors || []).map(a => ({ text: a.text, url: a.url })),
      };
